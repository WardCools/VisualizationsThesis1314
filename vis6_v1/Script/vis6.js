...
function visualizesvg2b(){
	...
	pointScale = d3.scale.linear()
		.domain([0, d3.max(usedTeamStats, function(d) { return Math.max(d[1][numberOfMatches][1],Math.max(d[1][numberOfMatches][3],d[1][numberOfMatches][5])); })])
		.range([0, barwidth]);
//	goalScale = d3.scale.linear()
//		.domain([0, d3.max(usedTeamStats, function(d) { return Math.max(d[1][numberOfMatches][3],d[1][numberOfMatches][5]); })])
//		.range([0, 200]);
	...
	goalsScored = psuedosvg2b.selectAll("rect.goalsscored")
		...
		.append("rect")
		.attr("x", function(d, i) {
	        return svg2width - rightPadding2 + 180 + Math.abs(pointScale(d[1][matchday][3]) - pointScale(d[1][Math.max(0,compMatchday)][3]));
		})
		.attr("y", function(d, i) {
			return yScale(20-i)-3;
		})
		.attr("width", function(d) {
			return pointScale(d[1][Math.max(0,compMatchday)][3]);	
		})
		.attr("opacity",0.4)
		.attr("height", 9)
		.attr("fill", "#008000");
	newGoalsScored = psuedosvg2b.selectAll("rect.newgoalsscored")
		...
		.attr("width", function(d) {
			return Math.abs(pointScale(d[1][matchday][3]) - pointScale(d[1][Math.max(0,compMatchday)][3]));
		})
		.attr("opacity",0.8)
		.attr("height", 9)
		.attr("fill", "#008000");
	goalsAgainst = psuedosvg2b.selectAll("rect.goalsagainst")
		...
		.append("rect")
		.attr("x", function(d, i) {
	        return svg2width - rightPadding2 + 180 + Math.abs(pointScale(d[1][matchday][5]) - pointScale(d[1][Math.max(0,compMatchday)][5]));
		})
		.attr("y", function(d, i) {
			return yScale(20-i) + 6;
		})
		.attr("width", function(d) {
			return pointScale(d[1][Math.max(0,compMatchday)][5]);	
		})
		.attr("opacity",0.4)
		.attr("height", 9)
		.attr("fill", "#FF0000");
	newGoalsAgainst = psuedosvg2b.selectAll("rect.goalsagainst")
		...
		.attr("width", function(d) {
			return Math.abs(pointScale(d[1][matchday][5]) - pointScale(d[1][Math.max(0,compMatchday)][5]));
		})
		.attr("opacity",0.8)
		.attr("height", 9)
		.attr("fill", "#FF0000");

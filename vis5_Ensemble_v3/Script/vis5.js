...
//		rightMatchday =  $("#dayslider").slider("values", 1);
		matchday = $("#dayslider").slider("values",1);
		compMatchday = $("#dayslider").slider("values", 0);
	}
	if(matchday < 4){
		leftMatchday = 0;
		rightMatchday = 8;
...
//	}
//	console.log("Pressed +");
	if(compMatchday < matchday){
		compMatchday++;
		buttonPushed = 1;
//		$( "#dayslider" ).slider("value", matchday );
		$( "#dayslider" ).slider("values", 0, compMatchday );
//		if(playing){
...
function button2Plus(){
	if(matchday < 38){
		matchday++;    		
		buttonPushed = 1;
		$( "#dayslider" ).slider("values", 1, matchday );
		if(playing){
...
function button2Minus(){
	if(matchday > compMatchday){
		matchday--;
		buttonPushed = 1;
		$( "#dayslider" ).slider("values", 1, matchdat );
	}
	console.log("Pressed -");
}
...
function visualizesvg2() {
	...
	psuedosvg2.append("rect")
		...
		.attr("y", yScale(0.5))
		.attr("width", 40)
		.attr("height", Math.abs(yScale(0.5)-yScale(20.7)))
		.attr("fill", "none")
		.attr("stroke", "#CC0000")
		.attr("opacity", 0.75)
		.attr("stroke-width", 5);
...
function setMatchdayText(){
	prevMatchdayText = svg1.append("text")
		.text("Left matchday: " + compMatchday)
		.attr("x", svg1width/2)
		.attr("y", topPadding + barHeight)
		.attr("font-family", "sans-serif")
		.attr("font-size", "30px")
		.attr("fill", "#CC0000")
		.attr("opacity", 0.55)
		.attr("font-weight", "bold")
		.attr("text-anchor", "middle");
	matchdayText = svg1.append("text")
		.text("Right matchday: " + matchday)
		.attr("x", svg1width/2)
		.attr("y", topPadding + barHeight+110)
		.attr("font-family", "sans-serif")
		.attr("font-size", "35px")
		.attr("fill", "#CC0000")
		.attr("opacity", 0.55)
...
//		});
	...
	teamNames.attr("fill", function(d){
    	if(contains(chosenTeams, d[0]) || contains(chosenTeams, "All")){
    		return "red";
    	}
    	else{

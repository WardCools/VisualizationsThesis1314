...
function visualize() {
	...
	for(var i = 0; i < numberOfMatches; i ++){
		if(d3.min(usedTeamStats, function(d) { return d[1][i][3] - d[1][i][5]; }) < xmin) xmin = d3.min(usedTeamStats, function(d) { return d[1][i][3] - d[1][i][5]; });
		if(d3.max(usedTeamStats, function(d) { return d[1][i][3] - d[1][i][5]; }) > xmax) xmax = d3.max(usedTeamStats, function(d) { return d[1][i][3] - d[1][i][5]; });
	}
...
//			})
	...
	teamlogos = svg.selectAll("image.logo")
				...
				.attr("width", logoSize)
		        .attr("x", function(d){
		        	return xScale(usedTeamStats[d[0]][1][d[1]][3] - usedTeamStats[d[0]][1][d[1]][5]) - logoSize/2;
		        })
		        .attr("y", function(d){
